...
const int daylightOffset_sec =
    3600; // Central European Summer Time (CEST) offset
...
void touchscreen_read(lv_indev_t *indev, lv_indev_data_t *data) {
    ...
    if (touchscreen.tirqTouched() && touchscreen.touched()) {
        ...
        // Calibrate Touchscreen points with map function to the correct width
        // and height
...
static void set_pwm_percent(float percent) {
    // Clamp command to a valid duty-cycle percentage before converting to timer
    // ticks.
    if (percent < 0.0f) {
        percent = 0.0f;
    }
...
static void compute_pid_and_drive_output(void) {
    // Fail-safe: if PID is disabled or sensor value is invalid, force output to
    // 0%.
    ...
    float pid_output =
        (g_kp * error) + (g_ki * g_pid_integral) + (g_kd * derivative);
...
static void control_slider_event_callback(lv_event_t *e) {
    ...
    // interactions.
    int control_temp_tenth =
        (int)lv_slider_get_value(slider); // 25.0 C to 45.0 C
    char temp_text[12];
...
static void vmc_switch_event_callback(lv_event_t *e) {
    ...
    if (!g_pid_enabled) {
        ...
        LV_LOG_USER("Manual VMC %s -> PWM %.0f%%", is_on ? "ON" : "OFF",
                    g_pwm_percent);
        return;
    }
...
static void refresh_external_html_tools() {
    String current_temp_text =
        isnan(g_current_temp_c) ? "--.-" : String(g_current_temp_c, 1);
...
void setup() {
    ...
    disp = lv_tft_espi_create(SCREEN_WIDTH, SCREEN_HEIGHT, draw_buf,
                              sizeof(draw_buf));
    // CYD is physically portrait but GUI layout is designed in landscape
    // coordinates.
