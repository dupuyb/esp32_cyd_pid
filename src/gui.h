...
static void update_dht_values(float temperature, float humidity) {
  ...
  if (g_label_ss_temp != NULL) {
    ...
    lv_obj_set_style_text_color(g_label_ss_temp, color_from_temperature(temperature), 0);
  }
}
...
void lv_create_gui() {
  ...
  // Page 2: screen saver (high contrast, large clock and process summary).
  LV_LOG_USER("Page 2");
  lv_obj_set_scrollbar_mode(tv2, LV_SCROLLBAR_MODE_OFF);

  lv_obj_t *panel_screensaver = lv_obj_create(tv2);
  ...
  lv_obj_set_style_text_color(g_label_ss_temp, isnan(g_current_temp_c) ? lv_color_hex(0x9CA3AF) : color_from_temperature(g_current_temp_c), 0);
