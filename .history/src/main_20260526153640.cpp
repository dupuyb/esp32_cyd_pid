...
static void update_access_network_labels(void) {
  ...
  if(g_label_mac_line != NULL) {
    ...
    if(show_mac) {
      String mac_text = "MAC: " + WiFi.macAddress();
...
static void set_pwm_percent(float percent) {
  ...
  if(g_led_pwm_state != NULL) {
    ...
    digitalWrite(VMC_ACTIVE_OUT_PIN, (duty > 0U) ? HIGH : LOW);
  }
...
void IRAM_ATTR frameTask(void *pvParameter) {
...
void setup() {
  ...
  xTaskCreate(&frameTask, "startingTcpTaskGet", 4096, NULL, TCP_TASK_PRIORITY, &tcpCxHandle);
