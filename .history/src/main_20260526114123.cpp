...
#define BACKLIGHT_PWM_PIN 21
#define BACKLIGHT_PWM_CHANNEL 1
#define BACKLIGHT_PWM_FREQUENCY_HZ 5000
#define BACKLIGHT_PWM_RESOLUTION_BITS 8
#define BACKLIGHT_PWM_MAX_DUTY ((1 << BACKLIGHT_PWM_RESOLUTION_BITS) - 1)
#define BACKLIGHT_MIN_PERCENT 20.0f
#define BACKLIGHT_MAX_PERCENT 100.0f
#define LIGHT_ADC_SAMPLES 8
...
static uint32_t g_last_backlight_update_ms = 0;
static uint32_t g_last_light_log_ms = 0;
static uint32_t g_light_zero_streak = 0;
static float g_filtered_light_raw = 0.0f;
...
static void update_backlight_from_ambient_light(bool force_update = false) {
  uint32_t now = millis();
  if(!force_update && (now - g_last_backlight_update_ms) < 200U) {
    return;
  }
  g_last_backlight_update_ms = now;

  uint32_t light_sum = 0;
  for(int i = 0; i < LIGHT_ADC_SAMPLES; ++i) {
    light_sum += (uint32_t)analogRead(LIGHT_SENSOR_PIN);
  }
  int light_raw = (int)(light_sum / LIGHT_ADC_SAMPLES);  // 0..4095 on ESP32 ADC
  if(light_raw <= 0) {
    ++g_light_zero_streak;
  } else {
    g_light_zero_streak = 0;
  }
  if(g_filtered_light_raw <= 0.0f) {
    g_filtered_light_raw = (float)light_raw;
  } else {
    g_filtered_light_raw = (g_filtered_light_raw * 0.85f) + ((float)light_raw * 0.15f);
  }

  float light_ratio = g_filtered_light_raw / 4095.0f;
  if(light_ratio < 0.0f) {
    light_ratio = 0.0f;
  }
  if(light_ratio > 1.0f) {
  ...
  // In shadow (low sensor value), backlight is reduced.
  float backlight_percent = BACKLIGHT_MIN_PERCENT + (light_ratio * (BACKLIGHT_MAX_PERCENT - BACKLIGHT_MIN_PERCENT));
  uint32_t backlight_duty = (uint32_t)((backlight_percent * (float)BACKLIGHT_PWM_MAX_DUTY / 100.0f) + 0.5f);
  ledcWrite(BACKLIGHT_PWM_CHANNEL, backlight_duty);

  if((now - g_last_light_log_ms) >= 1000U) {
    g_last_light_log_ms = now;
    Serial.printf("Light raw=%d filtered=%.0f ratio=%.2f backlight=%.0f%%\n", light_raw, g_filtered_light_raw, light_ratio, backlight_percent);
    if(g_light_zero_streak >= 5U) {
      Serial.println("Light warning: raw stays at 0. Check LDR wiring (use voltage divider), GND/3V3, and sensor output pin.");
    }
  }
}
...
void setup() {
  ...
  // Backlight PWM on CYD TFT BL pin.
  ledcSetup(BACKLIGHT_PWM_CHANNEL, BACKLIGHT_PWM_FREQUENCY_HZ, BACKLIGHT_PWM_RESOLUTION_BITS);
  ledcAttachPin(BACKLIGHT_PWM_PIN, BACKLIGHT_PWM_CHANNEL);
  update_backlight_from_ambient_light(true);
...
void loop() {
  update_dht_values();
  update_backlight_from_ambient_light();
